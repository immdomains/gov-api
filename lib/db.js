...
dotenv.config()
 ...
 class Db {
  ...
  async fetchUserByCookie(cookie) {
    const connection = await this.fetchConnection()
    const results = await connection.query('SELECT * FROM users WHERE cookie = ?', [
      cookie
    ])
    if (results.length === 0) {
      return null
  ...
  async fetchRecentUsers(limit = 100) {
    const connection = await this.fetchConnection()
    const results = await connection.query('SELECT * FROM users ORDER BY id DESC LIMIT ?', [
      limit
    ])
  ...
  async fetchScore0ApprovalsCount() {
    const connection = await this.fetchConnection()
    const results = await connection.query('SELECT COUNT(id) FROM users WHERE score0 >= ?', [
